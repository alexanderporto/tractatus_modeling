...
#   visPhysics(
#     enabled = TRUE,  # Keep physics enabled
#     solver = "barnesHut",  # Use Barnes-Hut solver for better performance
#     barnesHut = list(
#       gravitationalConstant = -3000,  # Adjust repulsion between nodes
#       centralGravity = 0.15,           # Pull nodes toward the center
#       springLength = 200,             # Adjust the length of edges
#       springConstant = 0.05,          # Adjust the stiffness of edges
#       damping = 0.1                   # Control the speed of the simulation
#     ),
#     stabilization = list(
#       enabled = TRUE,                 # Enable stabilization
#       iterations = 1000,              # Number of iterations to stabilize
...
# # Customize edges
# edges$width <- edges$weight * 1   # Scale edge width by weight
# edges$color <- "darkgrey"            # Set edge color
# edges$smooth <- TRUE             # Smooth edges for better visualization
# 
# # Render the interactive network graph with a legend and title
# visNetwork(nodes, edges) %>%
#   visOptions(highlightNearest = TRUE, nodesIdSelection = TRUE) %>%
#   visPhysics(
#     enabled = TRUE,  # Keep physics enabled
#     solver = "barnesHut",  # Use Barnes-Hut solver for better performance
#     barnesHut = list(
#       gravitationalConstant = -4000,  # Adjust repulsion between nodes
#       centralGravity = 0.15,           # Pull nodes toward the center
#       springLength = 200,             # Adjust the length of edges
#       springConstant = 0.05,          # Adjust the stiffness of edges
#       damping = 0.1                   # Control the speed of the simulation
...
# edges$smooth <- TRUE             # Smooth edges for better visualization
# 
# # Render the interactive network graph with forceAtlas2Based solver
# visNetwork(nodes, edges) %>%
#   visOptions(highlightNearest = TRUE, nodesIdSelection = TRUE) %>%
#   visPhysics(
#     enabled = TRUE,  # Keep physics enabled
...
#   htmlwidgets::prependContent(
#     htmltools::tags$div(
#       style = "font-family: Arial; font-size:16px; text-align: center; margin-bottom: 20px;",
#       "Tractatus Term Network"
#     )
#   ) %>%
#   # Disable physics after stabilization to prevent rotation
#   visEvents(
#     stabilizationIterationsDone = "function() {
#       this.setPhysicsEnabled(true);  // Disable physics after stabilization
#     }"
#   )
# 
# saveWidget(graph, file = "Tractatus_Term_Network.html", selfcontained = FALSE)
# 
